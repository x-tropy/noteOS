...
RUN aws s3 rm s3://$BUCKET_NAME/noteOS/vite/assets/ --endpoint-url $AWS_ENDPOINT_URL_S3 --recursive && \
    aws s3 cp ./public/vite/assets/ s3://$BUCKET_NAME/noteOS/vite/assets/ --recursive --endpoint-url $AWS_ENDPOINT_URL_S3 --include "*.js" --include "*.css" --exclude "*.js.*" --exclude "*.css.*"
