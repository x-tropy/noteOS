...
RUN aws s3 rm s3://$BUCKET_NAME/noteOS/vite/assets/ --endpoint-url $AWS_ENDPOINT_URL_S3 --recursive && \
    aws s3 cp ./public/vite/assets/ s3://$BUCKET_NAME/noteOS/vite/assets/ --recursive --endpoint-url $AWS_ENDPOINT_URL_S3 --exclude "*" --include "*.js" --include "*.css" --include "*.svg" --exclude "*.js*" --exclude "*.css*" --exclude "*.svg*"
